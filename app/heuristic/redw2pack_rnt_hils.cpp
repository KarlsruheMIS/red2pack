#include "../benchmark_red2pack_solver.h"
#include "../m2s_configuration_mis.h"
#include "../m2s_parse_parameters.h"
#include "../m2s_configuration_m2s.h"

#include <mis_config.h>
#include <red2pack-htwis-hils/algorithms/weighted_rnt_hils.h>

#include <fstream>
#include <iostream>

int main(int argc, char **argv) {
        using namespace red2pack;
        M2SConfig m2s_config;
        MISConfig mis_config;
        m2s_configuration_mis cfg;
        cfg.standard(mis_config);
        HilsConfig hils_config;
        std::string graph_filepath;

        // read arguments
        m2s_configuration_m2s configurator_m2s;
        configurator_m2s.standard_weighted(m2s_config);
        auto ret_code = cli::parse_parameters<cli::weighted_solver_cli>(argc, argv, m2s_config, graph_filepath);
        if (ret_code) return 0;

        mis_config.time_limit = m2s_config.time_limit;
        mis_config.seed = m2s_config.seed;
        mis_config.perform_reductions = m2s_config.perform_IS_reductions;

        // default values from ArgPack.cpp
        hils_config.iterations = 2000000;
        hils_config.p[0] = 2;
        hils_config.p[1] = 4;
        hils_config.p[2] = 4;
        hils_config.p[3] = 1;

        // seed
        hils_config.rand_seed = m2s_config.seed;

        // supress output
        std::streambuf *backup = std::cout.rdbuf();
        std::ofstream ofs;
        ofs.open("/dev/null");
        if (m2s_config.silent) {
                std::cout.rdbuf(ofs.rdbuf());
        }

        // benchmark
        ret_code = benchmark_red2pack_solver<weighted_rnt_hils>(graph_filepath, m2s_config, hils_config, mis_config);

        if (m2s_config.silent) {
                std::cout.rdbuf(backup);
        }

        return ret_code;
}
