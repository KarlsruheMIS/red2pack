...
unsigned int reduction_evolution<reducer>::perform_mis_search(MISConfig & mis_config, 
    ...
    std::cout << "The kernel size is: " << remaining_size << std::endl;
    /*
    ...
    if(time_now.count() > 0) {
    	std::cout << "Time for reductions: " << time_now.count() << " seconds." << std::endl; 
	std::cout << "\n"; 
    } else {
    	auto time_now_2 = std::chrono::duration_cast<std::chrono::milliseconds>(stop_point-start_point); 
	if(time_now_2.count() > 0) {
		std::cout << "Time for reductions: " << time_now_2.count() << " milliseconds." << std::endl; 
		std::cout << "\n"; 
	} else {
		auto time_now_3 = std::chrono::duration_cast<std::chrono::microseconds>(stop_point-start_point); 
		std::cout << "Time for reductions: " << time_now_3.count() << " microseconds." <<  std::endl; 
	       	std::cout << "\n"; 	
	}
    }
*/
