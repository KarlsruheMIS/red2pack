...
#include "red2pack/tools/macros.h"
...
class basicGraph {
       ...
       private:
        ...
        EdgeID number_of_two_edges() { return m2_edges.size(); }
        ...
        inline EdgeID get_first_link(const NodeID& node) { return m2_nodes[node].firstEdge; }

        inline EdgeID get_first_invalid_link(const NodeID& node) { return m2_nodes[node + 1].firstEdge; }

        // construction of the graph
        void start_construction(NodeID n, EdgeID m, EdgeID m2) {
                ...
                m2_nodes[node].firstEdge = e2;
        }

        EdgeID new_edge(NodeID source, NodeID target) {
                RED2PACK_ASSERT_TRUE(m_building_graph);
                RED2PACK_ASSERT_TRUE(e < m_edges.size());

                m_edges[e].target = target;
                EdgeID e_bar = e;
                ++e;

                RED2PACK_ASSERT_TRUE(source + 1 < m_nodes.size());
        ...
        EdgeID new_link(NodeID source, NodeID target) {
                RED2PACK_ASSERT_TRUE(m_building_graph);
                RED2PACK_ASSERT_TRUE(e2 < m2_edges.size());

                m2_edges[e2].target = target;
                EdgeID e2_bar = e2;
                ++e2;

                RED2PACK_ASSERT_TRUE(source + 1 < m2_nodes.size());
                m2_nodes[source + 1].firstEdge = e2;

                // fill isolated sources at the end
                if ((NodeID)(m2_last_source + 1) < source) {
                        for (NodeID i = source; i > (NodeID)(m2_last_source + 1); i--) {
                                m2_nodes[i].firstEdge = m2_nodes[m2_last_source + 1].firstEdge;
                        }
                }
                m2_last_source = source;
                return e2_bar;
        }
        ...
        void finish_construction() {
                ...
                if ((unsigned int)(m2_last_source) != node - 1) {
                        // in that case at least the last node was an isolated node
                        for (NodeID i = node; i > (unsigned int)(m2_last_source + 1); i--) {
                                m2_nodes[i].firstEdge = m2_nodes[m2_last_source + 1].firstEdge;
                        }
                }
        }
        ...
        EdgeID e2;     // current edge that is constructed
};

#ifndef forall_nodes
// todo: these macros are already defined by KaHIP/graph_access.h but our code should not be dependent on KaHIP
// work-around: copied macros directly from KaHIP
// better: use own macro names - or dispense macros
...
#define forall_out_links(G, e, n) \
        {                          \
                for (EdgeID e = G.get_first_link(n), end = G.get_first_invalid_link(n); e < end; ++e) {
...
class m2s_graph_access {
       ...
       public:
        ...
        /* build methods */
        /* ============================================================= */
        void start_construction(NodeID nodes, EdgeID edges, EdgeID two_edges);
        NodeID new_node();
        EdgeID new_edge(NodeID source, NodeID target);
        EdgeID new_link(NodeID source, NodeID target);
        void finish_construction();
        ...
        EdgeID number_of_links(); // returns number of 1-edges
        ...
        EdgeID get_first_link(NodeID node);
        EdgeID get_first_invalid_link(NodeID node);
        // =========================================================================
        ...
        EdgeWeight getLinkDegree(NodeID node);
        // ==================================================================================
        ...
        EdgeWeight getLinkWeight(EdgeID edge);
        void setLinkWeight(EdgeID edge, EdgeWeight weight);

        NodeID getEdgeTarget(EdgeID edge);

        // =================================================================================
        // we need the equivalent of getEdgeTarget for our macro equivalent
        NodeID getLinkTarget(EdgeID edge);
        // =================================================================================
        ...
        // builds canonical two neighborhood based on direct edges
        void construct_2neighborhood();
        void construct_2neighborhood_alternative();
...
/* graph build methods */
inline void m2s_graph_access::start_construction(NodeID nodes, EdgeID edges, EdgeID two_edges) {
        graphref->start_construction(nodes, edges, two_edges);
}
...
inline EdgeID m2s_graph_access::new_link(NodeID source, NodeID target) { return graphref->new_link(source, target); }
...
inline EdgeID m2s_graph_access::number_of_links() { return graphref->number_of_two_edges(); }
...
inline EdgeID m2s_graph_access::get_first_link(NodeID node) {
#ifdef NDEBUG
...
inline EdgeID m2s_graph_access::get_first_invalid_link(NodeID node) { return graphref->m2_nodes[node + 1].firstEdge; }
// ======================================================================
...
inline EdgeWeight m2s_graph_access::getLinkWeight(EdgeID edge) {
#ifdef NDEBUG
        return graphref->m2_edges[edge].weight;
#else
        return graphref->m2_edges.at(edge).weight;
#endif
}
...
inline void m2s_graph_access::setLinkWeight(EdgeID edge, EdgeWeight weight) {
#ifdef NDEBUG
        graphref->m2_edges[edge].weight = weight;
#else
        graphref->m2_edges.at(edge).weight = weight;
#endif
}
...
// ===============================================================
inline NodeID m2s_graph_access::getLinkTarget(EdgeID edge) {
#ifdef NDEBUG
...
inline EdgeWeight m2s_graph_access::getLinkDegree(NodeID node) {
...
inline void m2s_graph_access::copy(m2s_graph_access& G_bar) {
        G_bar.start_construction(number_of_nodes(), number_of_edges(), number_of_links());
        ...
        forall_nodes (ref, node) {
                ...
                forall_out_links (ref, e, node) {
                        NodeID target = getLinkTarget(e);
                        EdgeID shadow_edge = G_bar.new_link(shadow_node, target);
                        G_bar.setLinkWeight(shadow_edge, getLinkWeight(e));
                }
                endfor
        }
        endfor
...
inline void m2s_graph_access::construct_2neighborhood() {
        ...
        graphref->m2_nodes.resize(graphref->number_of_nodes() + 1);
	      for (NodeID v_idx = 0; v_idx < number_of_nodes(); v_idx++) {
                ...
                for (EdgeID j = graphref->m_nodes[v_idx].firstEdge; j < graphref->m_nodes[v_idx + 1].firstEdge; j++) {
                        touched.add(graphref->m_edges[j].target);
                }

                for (EdgeID k = graphref->m_nodes[v_idx].firstEdge; k < graphref->m_nodes[v_idx + 1].firstEdge; k++) {
        ...
        count_2_edges = 0;
        graphref->m2_edges.resize(count_2_edges + 1);  // guess for number of edges
        for (NodeID v_idx = 0; v_idx < number_of_nodes(); v_idx++) {
                graphref->m2_nodes[v_idx].firstEdge = count_2_edges;
                touched.add(v_idx);
	...
	if (count_2_edges > std::numeric_limits<EdgeID>::max()) {
		std::cout << "Cannot represent all 2-edges of graph with EdgeID!" << std::endl;
		exit(1);
	}
}
// ======================================================================================
