...
reduce_algorithm::reduce_algorithm(m2s_graph_access& G, const M2SConfig& m2s_config)
    : config(m2s_config),
      ...
      buffers(2, sized_vector<NodeID>(G.number_of_nodes())) {
        if (config.reduction_style == M2SConfig::Reduction_Style2::fast) {
...
void reduce_algorithm::reduce_graph_internal() {
        ...
        do {
                ...
                for (auto& reduction : global_status.reductions) {
                        active_reduction_index = reduction_map[static_cast<size_t>(reduction->get_reduction_type())];
...
void reduce_algorithm::build_solution(std::vector<bool>& solution_vec) {
...
void reduce_algorithm::get_exact_kernel(m2s_graph_access& reduced_graph, std::vector<NodeID>& reduced_node_id, std::vector<NodeID>& former_node_id) {
        ...
        NodeID reduced_edges = 0;
        NodeID reduced_two_edges = 0;

        for (size_t i = 0; i < status.n; i++) {
                if (!status.graph.hided_nodes[i]) {
                        reduced_node_id[i] = reduced_nodes;
                        former_node_id[reduced_nodes++] = i;

                        reduced_edges += status.graph[i].size();
                        reduced_two_edges += status.graph.get2neighbor_list(i).size();
                }
        }

        reduced_graph.start_construction(status.remaining_nodes, reduced_edges, reduced_two_edges);

        for (size_t i = 0; i < reduced_nodes; i++) {
                auto node = former_node_id[i];
                reduced_graph.new_node();
                reduced_graph.setNodeWeight(i, status.weights[node]);
                for (size_t j = 0; j < status.graph[node].size(); j++) {
                        auto target = status.graph[node][j];
                        //if (!status.graph.hided_nodes[target]) {
                ...
                for (size_t j = 0; j < status.graph.get2neighbor_list(node).size(); j++) {
                        auto target = status.graph.get2neighbor_list(node)[j];
                        //if (!status.graph.hided_nodes[target]) {
...
NodeWeight reduce_algorithm::get_solution_weight() const {
        return global_status.sol_weight;
}
