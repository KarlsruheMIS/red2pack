...
class weighted_rnt_chils : public rnt_solver_scheme {
       public:
        weighted_rnt_chils(std::unique_ptr<m2s_graph_access> G, M2SConfig m2s_cfg)
            : rnt_solver_scheme(std::move(G), std::move(m2s_cfg))
             {
                eps_time = 2; // works better with mmwis
        };
