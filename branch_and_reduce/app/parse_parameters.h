...
int parse_parameters(int argn, char **argv,
    ...
    struct arg_str *reduction_style2    = arg_str0(NULL, "reduction_style2", NULL, "Choose between initial, compact and extended.");
    struct arg_str *reduction_style     = arg_str0(NULL, "reduction_style", NULL, "Choose between normal/sparce (default), dense/osm.");
    ...
    /* struct arg_lit *disable_deg_one_e            = arg_lit0(NULL, "disable_deg_one_e","disable reduction deg_one_e"); */
    struct arg_lit *disable_clique               = arg_lit0(NULL, "disable_clique", "disable reduction clique");
    /* struct arg_lit *disable_clique_e             = arg_lit0(NULL, "disable_clique_e", "disable reduction clique_e"); */
    struct arg_lit *disable_cycle                = arg_lit0(NULL, "disable_cycle", "disable reduction cycle");
    /* struct arg_lit *disable_cycle_e              = arg_lit0(NULL, "disable_cycle_e", "disable reduction cycle_e"); */
    struct arg_lit *disable_neighborhood         = arg_lit0(NULL, "disable_neighborhood", "disable reduction neighborhood");
    /* struct arg_lit *disable_neighborhood_e       = arg_lit0(NULL, "disable_neighborhood_e", "disable reduction neighborhood_e"); */
    ...
    /* struct arg_lit *disable_domination_e         = arg_lit0(NULL, "disable_domination_e", "disable reduction domination_e"); */
    struct arg_lit *disable_fast_domination      = arg_lit0(NULL, "disable_fast_domination", "disable reduction fast_domination");
    /* struct arg_lit *disable_fast_domination_e    = arg_lit0(NULL, "disable_fast_domination_e", "disable reduction fast_domination_e"); */
...
/* able(argtable, s */
    ...
    if (user_conf->count > 0) {
        if (strcmp(user_conf->sval[0], "standard") == 0) cfg2.standard(m2s_config);
        else if (strcmp(user_conf->sval[0], "social") == 0) cfg2.social(m2s_config);
        else if (strcmp(user_conf->sval[0], "full_standard") == 0) cfg2.full_standard(m2s_config);
        else if (strcmp(user_conf->sval[0], "full_social") == 0) cfg2.full_social(m2s_config);
    }

    m2s_config.fullKernelization = false;
    if (kernelization_mode->count > 0) {
        if (strcmp(kernelization_mode->sval[0], "FastKer") == 0) m2s_config.fullKernelization = false;
        else if (strcmp(kernelization_mode->sval[0], "full") == 0) m2s_config.fullKernelization = true;
    }
