...
#include "ils/m2ps_local_search.h"

#include <algorithm>

#include "random_functions.h"

m2ps_local_search::m2ps_local_search() {
...
void m2ps_local_search::preprocess_graph(graph_access & G) {
    perm.construct(G); 
    build_candidates(G);
    onetight.clear();
    neighbors.clear();
...
void m2ps_local_search::preprocess_graph_with_candidates(graph_access & G, std::vector<NodeID> cand, unsigned int cand_size) {
    perm.construct(G); 
...
void m2ps_local_search::force(graph_access & G, unsigned int k) {
...
void m2ps_local_search::force_node(graph_access & G, NodeID node) {
...
void m2ps_local_search::simple_improvement(graph_access & G, bool forced, NodeID forced_node) {
...
void m2ps_local_search::direct_improvement(graph_access & G, bool forced, NodeID forced_node) {
...
void m2ps_local_search::insert_candidates(graph_access & G, std::vector<NodeID> cand, unsigned int num_cand) {
...
void m2ps_local_search::make_maximal(graph_access & G) {
...
void m2ps_local_search::build_onetight(NodeID node, graph_access & G) {
    onetight_size = 0; 
    forall_out_edges(G, edge, node) {
...
void m2ps_local_search::print_onetight() {
    for (unsigned int i = 0; i < onetight_size; ++i) {
        printf("Node: %d\n", onetight[i]);
    }
}

void m2ps_local_search::build_neighbors(NodeID node, graph_access & G) {
    neighbors_size = 0;
    forall_out_edges(G, edge, node) {
        NodeID target = G.getEdgeTarget(edge);
...
void m2ps_local_search::build_candidates(graph_access & G) {
    candidates.init(G.number_of_nodes());
...
void m2ps_local_search::update_candidates(NodeID node, graph_access & G) {
    forall_out_edges(G, edge, node) {
        NodeID target = G.getEdgeTarget(edge);
...
void m2ps_local_search::print_permutation() {
    perm.print(0);
