...
void m2ps_ils::perform_m2ps_ils(M2SConfig & config, graph_access & G, unsigned int iteration_limit) {
    reset();
    // Init operation log
...
void m2ps_ils::force(M2SConfig & config, graph_access & G, NodeID v, candidate_list *force_list) {
    // printf("Force %d\n", v);
...
void m2ps_ils::unwind(graph_access & G) {
    // printf("Unwind\n");
    operation_log::instance()->deactivate(); 
