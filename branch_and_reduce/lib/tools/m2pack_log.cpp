...
void m2pack_log::set_config(M2SConfig & config) {
    log_config = config; 
}

void m2pack_log::set_graph(M2S_GRAPH::graph_access & G) {
...
void m2pack_log::print_title() {
}

void m2pack_log::print_graph() {
...
void m2pack_log::print_round(M2SConfig & m2s_config) {
    filebuffer_string << std::endl;
...
void m2pack_log::print_repetition(M2SConfig & m2s_config) {
...
void m2pack_log::print_pool_title() {
...
void m2pack_log::print_evolution_title() {
...
void m2pack_log::print_init_title() {
...
void m2pack_log::print_separator() {
...
void m2pack_log::restart_total_timer() {
    total_timer.restart();
}

void m2pack_log::restart_online_timer() { online_timer.restart(); }

double m2pack_log::get_online_timer() { return online_timer.elapsed(); }

void m2pack_log::restart_timer() { t.restart(); }

double m2pack_log::get_timer() { return t.elapsed(); }

void m2pack_log::restart_evo_timer() {
    evo_timer.restart();
}

double m2pack_log::get_evo_timer() {
    return evo_timer.elapsed();
}

void m2pack_log::restart_operator_timer() {
    operator_timer.restart();
}

void m2pack_log::restart_building_pool_timer() {
    pool_timer.restart();
}

void m2pack_log::set_best_size(M2SConfig &m2s_config, unsigned int size) {
...
void m2pack_log::set_best_size_online(M2SConfig &m2s_config, unsigned int size) {
  if (size > best_solution_size) {
    best_solution_size = size;
...
double m2pack_log::get_pool_building_time() {
    double elapsed = pool_timer.elapsed();
    non_pool_timer.restart();
    return elapsed;
}

double m2pack_log::get_after_pool_time() {
    return non_pool_timer.elapsed();
}

void m2pack_log::inc_rounds() {
    number_of_rounds++;
}

void m2pack_log::inc_repetitions() {
    number_of_repetitions++; 
}

void m2pack_log::set_operator(std::string operator_name) {
...
void m2pack_log::set_result_operator(unsigned int result) {
...
void m2pack_log::set_avg_solution_size(double avg_size) {
    avg_solution_size = avg_size;
}

void m2pack_log::reset_best_size() {
    optimum_size = 0;
}
