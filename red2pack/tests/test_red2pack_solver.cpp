...
TEMPLATE_LIST_TEST_CASE("Testing M2S Solvers", "[template][m2s]", Solver_Types) {
        ...
        std::vector<std::string> instances = {"aGraphErdos40-8.graph", "lesmis.graph", "Outerplanar500_1.graph", "cac100.graph", "cond-mat-2005.graph"};
        double time_limit = 10;

        for(auto instance : instances) {

                INFO("Testing " + instance);
                M2SConfig m2sConfig;
                ...
                solver.solve();
                REQUIRE(is_maximal_2ps(graph, solver.get_solution(), solver.get_solution_size()));
        }
        ...
        /*for(auto instance : instances) {
                ...
                typename TestType::Solver solver(graph, m2sConfig, misConfig);
                auto info = TestType().info;

                BENCHMARK("Benchmark: " + instance + " for " + info) {
                    return solver.solve();
                };
        }*/
}
