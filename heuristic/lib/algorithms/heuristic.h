#ifndef INC_2_PACKING_SET_heuristic_H
#define INC_2_PACKING_SET_heuristic_H

#include "data_structures/m2s_graph_access.h"
#include "graph_access.h"
#include "mis_config.h"
#include "m2s_config.h"

namespace two_packing_set {

class heuristic {
public:
  /**
   * Constructor for 2-packing-set branch-and-reduce solver
   * @param graph
   * @param m2s_cfg
   * @param mis_cfg
   */
  heuristic(m2s_graph_access& G, M2SConfig m2s_cfg, MISConfig mis_cfg);

  /**
   * Find maximum 2-packing-set for graph
   * @return solved if true, false for timeout
   */
  bool run();

  NodeID get_solution_size() {
          return solution_size;
  }

  const std::vector<bool>& get_solution() {
          return solution_status;
  }

private:
  m2s_graph_access graph;
  M2SConfig m2s_cfg;
  MISConfig mis_cfg;

  std::vector<bool> solution_status;
  NodeID solution_size = 0;

  // buffers for kernel
  std::vector<NodeID> reduced_node_id;
  std::vector<NodeID> former_node_id;

  bool timeout = false;

  /**
   * Perform initial reductions
   */
  void perform_initial_reductions(graph_access& condensed_graph);
  void construct_condensed_graph(graph_access& condensed_graph);
};
} // namespace two_packing_set

#endif // INC_2_PACKING_SET_heuristic_H
